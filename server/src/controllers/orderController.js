...
const createOrder = async (req, res, next) => {
  try {
    if (req.user.emailVerified === false) {
      return res.status(403).json({ error: 'EMAIL_NOT_VERIFIED', message: 'Please verify your email before placing an order' })
    }
    ...
    // transaction ensures stock decrement and order creation are atomic
    ...
    // pre-check before opening transaction to fail fast
