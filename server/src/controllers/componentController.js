...
const getComponents = async (req, res, next) => {
  try {
    ...
    if (Object.keys(specFilters).length === 0) {
      // no spec filters — use DB pagination directly
    ...
    // prisma can't filter on JSON fields, so fetch all and filter in JS
    ...
    for (const { category, name, manufacturer, color, specData } of allComponents) {
      ...
      for (const field of fields) {
        ...
        if (field === 'integratedGraphics') {
          ...
          continue // integratedGraphics is the only boolean shown; skip the rest
